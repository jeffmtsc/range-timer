...
5. Tap **Add to Home Screen** from the app's own menu (gear icon &#9881; on the home screen), or the browser menu (⋮) → **Add to Home screen** / **Install app** if the in-app option isn't shown yet (Chrome only offers it once it decides the site is install-eligible, which can take a moment on first visit).
...
- **Add to Home Screen** — the same gear-icon (&#9881;) menu on the home screen offers this at the top when the browser has made its install prompt available (Chrome/Android decides this itself, usually within a visit or two — not necessarily the very first). Tapping it triggers the normal Android install dialog. If it's not showing yet, use the browser's own menu (⋮) → Add to Home screen instead — see the install steps above.
