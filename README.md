...
- **Timer screen**:
  ...
  5. When the whole match is done, BEGIN greys out so you can't accidentally re-fire it — **Next →** is the only way forward (Repeat still works if you deliberately want to redo the match). If the next match is at a different distance/position, or this was the last match in the course of fire, the screen itself immediately shows **CEASE FIRE — unload and show clear**, telling you to hold there until RSOs confirm all firearms are safe before tapping Next. Tapping Next then walks through **Score & Change Targets** if the course of fire calls for one, then **The Range is Safe** ("You may move to [distance] for [next match]"), or "That concludes your competition" if it was the last match. If the next match is at the same distance/position, the screen just shows plain **COMPLETE** and Next skips straight to Score & Change (if any) and the next match — no need to re-announce moving.
