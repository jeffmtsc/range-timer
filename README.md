...
- **Timer screen**:
  ...
  5. **Repeat** restarts the current string/attempt from the briefing if you need to redo it. **Next →** ends the match. If the next match is at a different distance/position — or this was the last match in the course of fire — you're walked through: **Cease Fire** ("Unload and show clear," RSOs confirm firearms safe) → **Score & Change Targets** if the course of fire calls for one → **The Range is Safe** ("You may move to [distance] for [next match]"), or **Competition Complete** ("That concludes your competition") if it was the last match. If the next match is at the same distance/position, it skips straight to Score & Change (if any) and the next match — no need to re-announce moving.
